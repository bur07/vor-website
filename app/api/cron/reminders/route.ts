...
export async function GET(req: Request) {
  ...
  // ── Unsigned quote follow-ups (3+ days old, no signature) ──
  const THREE_DAYS = 3 * 24 * 60 * 60 * 1000
  const followUps = all.filter(a =>
    !a.clientSignedAt &&
    !a.paidAt &&
    !a.cancelledAt &&
    !a.followUpSentAt &&
  ...
  const followUpResults: { refCode: string; email: boolean }[] = []
  ...
  <div style="height:3px;background:#c9a84c"></div>
  <div style="padding:36px 40px">
    <p style="font-size:15px;line-height:1.8;color:#2c2c2c;margin:0 0 8px">Hi ${a.clientName ?? 'there'},</p>
    ...
    <div style="text-align:center;margin:0 0 28px">
      <a href="https://vorwindowco.com/sign/${a.refCode}"
         style="display:inline-block;background:#1B3A5C;color:#f5f0e8;font-size:14px;font-family:-apple-system,sans-serif;font-weight:600;letter-spacing:.06em;text-decoration:none;padding:14px 36px;border-radius:6px;">
        Review &amp; Sign Quote →
      </a>
    </div>
