// Netlify serverless function — quote & booking email handler
...
function bizFooter() {
  return `<div style="margin-top:36px;padding-top:20px;border-top:1px solid #B8D5EF;font-size:11px;color:#6A8296;letter-spacing:0.08em">
      VØR Window Co. · Sydney & ACT · info@vorwindowco.com
    </div>
  </div>`
}

// Gold-accented client email helpers
...
function refBox(refCode) {
  return `<div style="background:#c9a84c;padding:22px;text-align:center;margin:24px 0">
    ...
    <p style="font-size:34px;font-weight:700;letter-spacing:0.2em;color:#fff;margin:0;font-family:Georgia,serif">${refCode}</p>
    <p style="font-size:11px;color:rgba(255,255,255,0.72);margin:7px 0 0">You'll need this code to view and book your service</p>
  </div>`
}

// ─── Quote emails ───────────────────────────────────────────
function buildBizQuoteHtml(d) {
  return bizHeader('New Quote Request — ' + d.refCode) +
    `<table style="width:100%;border-collapse:collapse">` +
    bizRow('Reference', `<strong>${d.refCode}</strong>`) +
    bizRow('Name', d.name) +
    bizRow('Email', d.email) +
    bizRow('Phone', d.phone) +
    bizRow('Address', d.address) +
    bizRow('Property Type', d.propertyType) +
    bizRow('Property Size', d.propertySize) +
    bizRow('Storeys', d.storeys) +
    bizRow('Inspection Date', d.inspectionDate) +
    bizRow('Inspection Time', d.inspectionTime) +
    bizRow('Service Area', d.serviceArea) +
    bizRow('Special Requirements', d.specialRequirements) +
    `</table>` +
    bizFooter()
}

function buildClientQuoteHtml(d) {
  const inner = `
    <p style="font-size:15px;line-height:1.8;color:#2c2c2c;margin:0 0 10px">Hi ${d.name},</p>
    <p style="font-size:14px;line-height:1.9;color:#5a4a2a;margin:0 0 24px">Thank you for your quote request. We've received all your details and will prepare a personalised quote within 24 hours.</p>
    ${refBox(d.refCode)}
  ...
  return clientWrap(inner)
}

// ─── Booking emails ─────────────────────────────────────────
function buildBizBookingHtml(d) {
  return bizHeader('New Booking Confirmed — ' + d.refCode) +
    `<table style="width:100%;border-collapse:collapse">` +
    bizRow('Reference', `<strong>${d.refCode}</strong>`) +
    bizRow('Name', d.name) +
    bizRow('Email', d.email) +
    bizRow('Phone', d.phone) +
    bizRow('Service Tier', d.tier) +
    bizRow('Total Price', `$${d.price} AUD`) +
    bizRow('Payment Type', d.paymentType) +
    bizRow('Amount Paid', `$${d.amountPaid} AUD`) +
    bizRow('Balance Due', `$${d.balanceDue} AUD`) +
    bizRow('Appointment Date', d.date) +
    bizRow('Appointment Time', d.time) +
    (d.note ? bizRow('Quote Note', d.note) : '') +
    `</table>` +
    bizFooter()
}

function buildClientBookingHtml(d) {
  const inner = `
    <p style="font-size:15px;line-height:1.8;color:#2c2c2c;margin:0 0 10px">Hi ${d.name},</p>
    <p style="font-size:14px;line-height:1.9;color:#5a4a2a;margin:0 0 24px">Your booking is confirmed. Here's a summary of everything for your records.</p>
    <div style="border-left:3px solid #c9a84c;margin:0 0 24px">
      <table style="width:100%;border-collapse:collapse">
        ...
        ${d.balanceDue > 0 ? clientRow('Balance Due on Day', `$${d.balanceDue} AUD`) : ''}
        ${d.note ? clientRow('Note', d.note) : ''}
      </table>
    </div>
    <p style="font-size:13px;line-height:1.8;color:#5a4a2a;margin:0 0 20px">Noah will be in touch to confirm your appointment time. If you need to make any changes, please reply to this email or call directly.</p>
    <p style="font-size:12px;color:#8a7a5a;margin:0">
      <a href="tel:+61416572468" style="color:#c9a84c;text-decoration:none">0416 572 468</a> · <a href="mailto:info@vorwindowco.com" style="color:#c9a84c;text-decoration:none">info@vorwindowco.com</a>
    </p>`
  return clientWrap(inner)
}
    ...
    if (d.type === 'quote') {
      await Promise.all([
        resend.emails.send({
          ...
          to: BUSINESS_EMAIL,
          reply_to: d.email,
          subject: `[${d.refCode}] New quote request — ${d.name}`,
          html: buildBizQuoteHtml(d),
        }),
        resend.emails.send({
          from: FROM,
          to: d.email,
          reply_to: BUSINESS_EMAIL,
          subject: `[${d.refCode}] Your VØR Window Co. quote request`,
          html: buildClientQuoteHtml(d),
        }),
      ])
      ...
      await Promise.all([
        resend.emails.send({
          ...
          to: BUSINESS_EMAIL,
          reply_to: d.email,
          subject: `[${d.refCode}] New booking — ${d.name} · ${d.tier}`,
          html: buildBizBookingHtml(d),
        }),
        resend.emails.send({
          from: FROM,
          to: d.email,
          reply_to: BUSINESS_EMAIL,
          subject: `[${d.refCode}] Your VØR booking is confirmed`,
          html: buildClientBookingHtml(d),
        }),
      ])
    } else {
